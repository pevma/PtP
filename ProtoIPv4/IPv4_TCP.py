...
class pacifyIpv4Tcp:
  ...
  def writeIPv4TcpRule(self, sid_id_tcp, tcp_content_match_string, \
  directory, src_name):
    ##creating and writing a sid.rules file
    rule_file = open('%s/%s.rules' % (directory,sid_id_tcp), 'w+')
    
    content_all_ready_for_rule = None
    content_all_ready_for_rule = ""
    
    if (len(tcp_content_match_string) > 250):
      
      content_tcp_all_array = [tcp_content_match_string[i:i+250] for i in range(0, len(tcp_content_match_string), 250)] 
      
      for i in content_tcp_all_array:
      ...
      tcp_content_match_string = tcp_content_match_string.replace('|', '|7C|').\
      replace('"', '|22|').replace(';', '|3B|').replace(':', '|3A|').\
      replace(' ', '|20|').replace('\\', '|5C|').replace('\'', '|27|').\
      replace('\r', '|0d|').replace('\n', '|0a|')
      
      content_all_ready_for_rule = \
      ("content:\"%s\"; " % (tcp_content_match_string))
      
    
    
    rule_file.write ( \
    "alert tcp any any -> any any (msg:\"TCP tests - sid %s , pcap - %s \"; \
    %s reference:url,%s; sid:%s; rev:1;)" % \
    (sid_id_tcp, sid_id_tcp, content_all_ready_for_rule, src_name, sid_id_tcp) )
    rule_file.close()

  def rebuildIPv4TcpSessionExtraTcpSAs(self, packet, results_directory, \
  sid_id_tcp, src_name, repo_name):
    ...
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Tcp_Extra_SAs_before_Real_SA-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    ...
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Tcp_Extra_SAs_before_Fragmented_Real_SA_Ordered-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Tcp_Extra_SAs_before_Fragmented_Real_SA_Reversed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Tcp_Extra_SAs_before_Fragmented_Real_SA_Mixed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Tcp_Extra_SAs_after_Real_SA-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Tcp_Extra_SAs_after_Fragmented_Real_SA_Ordered-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Tcp_Extra_SAs_after_Fragmented_Real_SA_Reversed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Tcp_Extra_SAs_after_Fragmented_Real_SA_Mixed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Tcp_Extra_SAs_before_and_after_Real_SA-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Tcp_Extra_SAs_before_and_after_Fragmented_Real_SA_Ordered-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Tcp_Extra_SAs_before_and_after_Fragmented_Real_SA_Reversed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Tcp_Extra_SAs_before_and_after_Fragmented_Real_SA_Mixed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
  ...
  def rebuildIPv4TcpSession(self, packet, results_directory, sid_id_tcp, \
  src_name, repo_name):
    session_packets = list()
    session_packets_fragmented = list()
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list


  def rebuildIPv4TcpSessionDot1Q(self, packet, results_directory, \
  sid_id_tcp, src_name, repo_name):
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered_Dot1Q-%s-tp-01.pcap"\
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed_Dot1Q-%s-tp-01.pcap"\
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list


  def rebuildIPv4TcpSessionDot1QWrongTagInFragments(self, packet, \
  results_directory, \
  sid_id_tcp, src_name, repo_name):
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered_Dot1Q_data_tag_missing_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed_Dot1Q_data_tag_missing_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed_Dot1Q_data_tag_missing_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered_Dot1Q_data_tag_one_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed_Dot1Q_data_tag_one_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed_Dot1Q_data_tag_one_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
  ...
  def rebuildIPv4TcpSessionQinQ(self, packet, results_directory, sid_id_tcp, \
  src_name, repo_name):
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list

  def rebuildIPv4TcpSessionQinQWrongTagInFragments(self, packet, \
  results_directory, sid_id_tcp, src_name, repo_name):
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed_QinQ_data_frag_wrong_dot1q_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed_QinQ_data_frag_wrong_dot1ad_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Ordered_QinQ_data_frag_one_tagged_fragments-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Reversed_QinQ_data_frag_one_tagged_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Fragmented_Mixed_QinQ_data_frag_one_tagged_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
  ...
  def rebuildIPv4TcpSeqOverSpill(self, packet, results_directory, \
  sid_id_tcp, src_name, repo_name):
    ...
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    ...
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    
    
  def rebuildIPv4TcpSeqOverSpillDot1Q(self, packet, results_directory, \
  sid_id_tcp, src_name, repo_name):
    ...
    session_packets_seq_overspill.append(finAck)
    session_packets_seq_overspill.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill)
    session_packets_seq_overspill[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill.append(finAck)
    session_packets_seq_overspill.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill)
    session_packets_seq_overspill[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill.append(finAck)
    session_packets_seq_overspill.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill)
    session_packets_seq_overspill[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list

  def rebuildIPv4TcpSeqOverSpillDot1QWrongTagInFragments(self, packet, \
  results_directory, sid_id_tcp, src_name, repo_name):
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_Dot1Q_data_tag_missing_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_Dot1Q_data_tag_missing_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_Dot1Q_data_tag_missing_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_Dot1Q_data_tag_one_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_Dot1Q_data_tag_one_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_Dot1Q_data_tag_one_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
  ...
  def rebuildIPv4TcpSeqOverSpillQinQ(self, packet, results_directory, \
  sid_id_tcp, src_name, repo_name):
    ...
    session_packets_seq_overspill.append(finAck)
    session_packets_seq_overspill.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill)
    session_packets_seq_overspill[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill.append(finAck)
    session_packets_seq_overspill.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill)
    session_packets_seq_overspill[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill.append(finAck)
    session_packets_seq_overspill.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill)
    session_packets_seq_overspill[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill.append(finAck)
    session_packets_seq_overspill.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_QinQ_data_tag_only_dot1q-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill)
    session_packets_seq_overspill[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_tag_only_dotq-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_tag_only_dot1q-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_tag_only_dot1q-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list


  def rebuildIPv4TcpSeqOverSpillQinQWrongTagInFragments(self, packet, \
  results_directory, sid_id_tcp, src_name, repo_name):
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_frag_only_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_frag_only_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_frag_only_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_frag_only_one_tagged_in_fragments-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_frag_only_one_tagged_in_fragments-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_frag_only_one_tagged_in_fragments-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list


  def midstreamIPv4Tcp(self, fragit, results_directory, sid_id_tcp, \
  src_name, repo_name):
    ...
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Ordered-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    ...
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Reversed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(fragit_done)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Mixed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Regular'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    
  def midstreamIPv4TcpDot1Q(self, fragit, results_directory, sid_id_tcp, \
  src_name, repo_name):
    ...
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit)

    fragit_done = fragment(fragit, fragsize=10 )
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Ordered_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    ...
    fragit_done.reverse()
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Reversed_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(fragit_done)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Mixed_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    
  def midstreamIPv4TcpDot1QWrongTagInFragments(self, fragit, \
  results_directory, sid_id_tcp, src_name, repo_name):
    ...
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Ordered_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Reversed_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done_wrong_dot1q_tag)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(fragit_done_wrong_dot1q_tag)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Mixed_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done_wrong_dot1q_tag)
    ...
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Ordered_Dot1Q_data_tag_none_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Reversed_Dot1Q_data_tag_none_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done_no_dot1q_tag)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(fragit_done_no_dot1q_tag)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Mixed_Dot1Q_data_tag_none_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done_no_dot1q_tag)


  def midstreamIPv4TcpQinQ(self, fragit, results_directory, sid_id_tcp, \
  src_name, repo_name):
    ...
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit)

    fragit_done = fragment(fragit, fragsize=10 )
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Ordered_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    ...
    fragit_done.reverse()
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Reversed_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(fragit_done)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Mixed_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    
  def midstreamIPv4TcpQinQWrongTagInFragments(self, fragit, \
  results_directory, sid_id_tcp, src_name, repo_name):
    #Wrongly tagged fragments 
    #Using DOUBLE VLAN Tagging - QinQ
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Ordered_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Reversed_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_dot1ad)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_wrong_dot1ad)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Mixed_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_dot1ad)
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Ordered_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Reversed_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_dot1q)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_wrong_dot1q)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Mixed_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_dot1q)
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Ordered_QinQ_data_frag_only_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Reversed_QinQ_data_frag_only_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_only_dot1q)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_only_dot1q)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Mixed_QinQ_data_frag_only_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_only_dot1q)
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Ordered_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Reversed_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_both)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_wrong_both)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Mixed_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_both)
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Ordered_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Reversed_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_missing_tags)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_missing_tags)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Mixed_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_missing_tags)
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Ordered_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Reversed_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_reversed_tags)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_reversed_tags)
    wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream_Fragmented_Mixed_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_reversed_tags)

  def reconstructIPv4TcpPacket(self, packet):
    ...
    dport=55555, seq=packet.seq, ack=packet.ack) \
    /"RANDOM data 12345 - 007 Sexy and You know it 007"/packet[TCP][Raw]
  ...
  def tcpReWrite(self, scapy_load, FN, pcap_id, results_directory, \
  source_name, sid_id_tcp, tcp_str, repository_name):
    
    # writing the http (tcp to be) request packet to pcap
    # in regression script format
    # 2002031-001-sandnet-public-tp-01.pcap - example
    ## 001 - starts here ##
    
    ipv4_tcp_packet = self.reconstructIPv4TcpPacket(scapy_load[FN])
    
    if Global_Vars.yaml_options['Protocols']['TCP']['WriteRule']:
      self.writeIPv4TcpRule(sid_id_tcp, tcp_str, \
      os.path.join(results_directory, 'Rules'), source_name)
    
    if Global_Vars.yaml_options['Protocols']['TCP']['Midstream']['Midstream']:
      wrpcap("%s/%s-%s-%s_IPv4_TCP_Midstream-%s-tp-01.pcap" \
      % (os.path.join(results_directory, 'Midstream', 'Regular'), sid_id_tcp, self.incrementPcapId("byOne"), \
      source_name, repository_name) , ipv4_tcp_packet)
      
      self.midstreamIPv4Tcp(ipv4_tcp_packet, results_directory, sid_id_tcp, \
      source_name, repository_name)
      
      self.writeIPv4TcpRule(sid_id_tcp, tcp_str, \
      os.path.join(results_directory, 'Midstream', 'Regular'), source_name)
    
    if Global_Vars.yaml_options['Protocols']['TCP']['Midstream']['Dot1Q']:
      self.midstreamIPv4TcpDot1Q(ipv4_tcp_packet, results_directory, \
      sid_id_tcp, source_name, repository_name)
      
      self.midstreamIPv4TcpDot1QWrongTagInFragments(ipv4_tcp_packet, \
      results_directory, sid_id_tcp, source_name, repository_name)
      
      self.writeIPv4TcpRule(sid_id_tcp, tcp_str, \
      os.path.join(results_directory, 'Midstream', 'Dot1Q'), source_name)
      
    
    if Global_Vars.yaml_options['Protocols']['TCP']['Midstream']['QinQ']:
      self.midstreamIPv4TcpQinQ(ipv4_tcp_packet, results_directory, \
      sid_id_tcp, source_name, repository_name)
      
      self.midstreamIPv4TcpQinQWrongTagInFragments(ipv4_tcp_packet, \
      results_directory, sid_id_tcp, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Session']['Session']:
      self.rebuildIPv4TcpSession(ipv4_tcp_packet, results_directory, \
      sid_id_tcp, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Session']['ExtraTcpSA']:
      self.rebuildIPv4TcpSessionExtraTcpSAs(ipv4_tcp_packet, \
      results_directory, sid_id_tcp, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Session']['Dot1Q']:
      self.rebuildIPv4TcpSessionDot1Q(ipv4_tcp_packet, results_directory, \
      sid_id_tcp, source_name, repository_name)
      
      self.rebuildIPv4TcpSessionDot1QWrongTagInFragments(ipv4_tcp_packet, \
      results_directory, sid_id_tcp, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Session']['QinQ']:
      self.rebuildIPv4TcpSessionQinQ(ipv4_tcp_packet, results_directory, \
      sid_id_tcp, source_name, repository_name)
      
      self.rebuildIPv4TcpSessionQinQWrongTagInFragments(ipv4_tcp_packet, \
      results_directory, sid_id_tcp, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Session']['SeqOverspill']:
      self.rebuildIPv4TcpSeqOverSpill(ipv4_tcp_packet, results_directory, \
      sid_id_tcp, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Session']['Dot1Q']:
      self.rebuildIPv4TcpSeqOverSpillDot1Q(ipv4_tcp_packet, results_directory, \
      sid_id_tcp, source_name, repository_name)
      
      self.rebuildIPv4TcpSeqOverSpillDot1QWrongTagInFragments(ipv4_tcp_packet, \
      results_directory, sid_id_tcp, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Session']['QinQ']:
      self.rebuildIPv4TcpSeqOverSpillQinQ(ipv4_tcp_packet, \
      results_directory, sid_id_tcp, source_name, repository_name)
      
      self.rebuildIPv4TcpSeqOverSpillQinQWrongTagInFragments(ipv4_tcp_packet, \
      results_directory, sid_id_tcp, source_name, repository_name)
  ...
  source_name, sid_id_tcp, tcp_str, repository_name):
    ...
    self.sid_id_tcp_to_pass = sid_id_tcp
    self.tcp_str_to_pass = tcp_str
    self.repository_name_to_pass = repository_name
    
    # if TCP over IPv4 is enabled in yaml
    if Global_Vars.yaml_options['Protocols']['TCP']['IPv4']:
      self.tcpReWrite( \
      self.scapy_load_to_pass, self.FN_to_pass, self.pcap_id_to_pass, \
      self.results_directory_to_pass, self.source_name_to_pass, \
      self.sid_id_tcp_to_pass, self.tcp_str_to_pass, \
