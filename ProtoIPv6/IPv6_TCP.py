...
class pacifyIpv6Tcp:
  ...
  def writeIPv6TcpRule(self, sid_id_tcp_v6, tcp_content_match_string, \
  directory, src_name):
    ##creating and writing a sid.rules file
    rule_file = open('%s/%s.rules' % (directory,sid_id_tcp_v6), 'w+')
    ...
    (sid_id_tcp_v6, sid_id_tcp_v6, content_all_ready_for_rule, src_name, \
    sid_id_tcp_v6) )
    
    rule_file.close()

  def rebuildIPv6TcpSessionExtraTcpSAs(self, packet, results_directory, \
  sid_id_tcp_v6, src_name, repo_name):
    ...
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Tcp_Extra_SAs_before_Real_SA-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    ...
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Tcp_Extra_SAs_before_Fragmented_Real_SA_Ordered-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Tcp_Extra_SAs_before_Fragmented_Real_SA_Reversed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Tcp_Extra_SAs_before_Fragmented_Real_SA_Mixed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Tcp_Extra_SAs_after_Real_SA-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Tcp_Extra_SAs_after_Fragmented_Real_SA_Ordered-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Tcp_Extra_SAs_after_Fragmented_Real_SA_Reversed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Tcp_Extra_SAs_after_Fragmented_Real_SA_Mixed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Tcp_Extra_SAs_before_and_after_Real_SA-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Tcp_Extra_SAs_before_and_after_Fragmented_Real_SA_Ordered-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Tcp_Extra_SAs_before_and_after_Fragmented_Real_SA_Reversed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Tcp_Extra_SAs_before_and_after_Fragmented_Real_SA_Mixed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
  ...
  def rebuildIPv6TcpSession(self, packet, results_directory, \
  sid_id_tcp_v6, src_name, repo_name):
    session_packets = list()
    session_packets_fragmented = list()
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list


  def rebuildIPv6TcpSessionDot1Q(self, packet, results_directory, \
  sid_id_tcp_v6, src_name, repo_name):
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Dot1Q-%s-tp-01.pcap" 
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered_Dot1Q-%s-tp-01.pcap"\
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed_Dot1Q-%s-tp-01.pcap"\
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list


  def rebuildIPv6TcpSessionDot1QWrongTagInFragments(self, packet, \
  results_directory, sid_id_tcp_v6, src_name, repo_name):
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered_Dot1Q_data_tag_missing_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed_Dot1Q_data_tag_missing_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed_Dot1Q_data_tag_missing_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered_Dot1Q_data_tag_one_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed_Dot1Q_data_tag_one_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed_Dot1Q_data_tag_one_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
  ...
  def rebuildIPv6TcpSessionQinQ(self, packet, results_directory, \
  sid_id_tcp_v6, src_name, repo_name):
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_QinQ-%s-tp-01.pcap" 
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(finAck)
    session_packets.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list

  def rebuildIPv6TcpSessionQinQWrongTagInFragments(self, packet, \
  results_directory, sid_id_tcp_v6, src_name, repo_name):
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed_QinQ_data_frag_wrong_dot1q_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed_QinQ_data_frag_wrong_dot1ad_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Ordered_QinQ_data_frag_one_tagged_fragments-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Reversed_QinQ_data_frag_one_tagged_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(finAck)
    session_packets_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Fragmented_Mixed_QinQ_data_frag_one_tagged_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
  ...
  def rebuildIPv6TcpSeqOverSpill(self, packet, results_directory, \
  sid_id_tcp_v6, src_name, repo_name):
    ...
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    ...
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    
    
  def rebuildIPv6TcpSeqOverSpillDot1Q(self, packet, results_directory, \
  sid_id_tcp_v6, src_name, repo_name):
    ...
    session_packets_seq_overspill.append(finAck)
    session_packets_seq_overspill.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill)
    session_packets_seq_overspill[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill.append(finAck)
    session_packets_seq_overspill.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill)
    session_packets_seq_overspill[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_Dot1Q_tagged_wrong-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill.append(finAck)
    session_packets_seq_overspill.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill)
    session_packets_seq_overspill[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_Dot1Q_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list

  def rebuildIPv6TcpSeqOverSpillDot1QWrongTagInFragments(self, packet, \
  results_directory, sid_id_tcp_v6, src_name, repo_name):
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_Dot1Q_data_tag_missing_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_Dot1Q_data_tag_missing_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_Dot1Q_data_tag_missing_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_Dot1Q_data_tag_one_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_Dot1Q_data_tag_one_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_Dot1Q_data_tag_one_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
  ...
  def rebuildIPv6TcpSeqOverSpillQinQ(self, packet, results_directory, \
  sid_id_tcp_v6, src_name, repo_name):
    ...
    session_packets_seq_overspill.append(finAck)
    session_packets_seq_overspill.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill)
    session_packets_seq_overspill[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill.append(finAck)
    session_packets_seq_overspill.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill)
    session_packets_seq_overspill[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_tags_reversed-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill.append(finAck)
    session_packets_seq_overspill.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill)
    session_packets_seq_overspill[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_tag_missing-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill.append(finAck)
    session_packets_seq_overspill.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_QinQ_data_tag_only_dot1q-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill)
    session_packets_seq_overspill[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_tag_only_dotq-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_tag_only_dot1q-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_tag_only_dot1q-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list


  def rebuildIPv6TcpSeqOverSpillQinQWrongTagInFragments(self, packet, \
  results_directory, sid_id_tcp_v6, src_name, repo_name):
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_frag_only_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_frag_only_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_frag_only_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Ordered_QinQ_data_frag_only_one_tagged_in_fragments-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Reversed_QinQ_data_frag_only_one_tagged_in_fragments-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list
    ...
    session_packets_seq_overspill_fragmented.append(finAck)
    session_packets_seq_overspill_fragmented.append(finalAck)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Session_Seq_Overspill_Fragmented_Mixed_QinQ_data_frag_only_one_tagged_in_fragments-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_seq_overspill_fragmented)
    session_packets_seq_overspill_fragmented[:] = [] #empty the list


  def midstreamIPv6Tcp(self, fragit, results_directory, \
  sid_id_tcp_v6, src_name, repo_name):
    
    
    fragit_done = fragment6(fragit, 74 )
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Ordered-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    ...
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Reversed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(fragit_done)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Mixed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    
  def midstreamIPv6TcpDot1Q(self, fragit, results_directory, \
  sid_id_tcp_v6, src_name, repo_name):
    ...
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit)

    fragit_done = fragment6(fragit, 74 )
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Ordered_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Reversed_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(fragit_done)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Mixed_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    
  def midstreamIPv6TcpDot1QWrongTagInFragments(self, fragit, \
  results_directory, sid_id_tcp_v6, src_name, repo_name):
    ...
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Ordered_Dot1Q_data_tag_wrong_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Reversed_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done_wrong_dot1q_tag)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(fragit_done_wrong_dot1q_tag)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Mixed_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done_wrong_dot1q_tag)
    ...
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Ordered_Dot1Q_data_tag_none_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Reversed_Dot1Q_data_tag_none_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done_no_dot1q_tag)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(fragit_done_no_dot1q_tag)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Mixed_Dot1Q_data_tag_none_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done_no_dot1q_tag)


  def midstreamIPv6TcpQinQ(self, fragit, results_directory, \
  sid_id_tcp_v6, src_name, repo_name):
    ...
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit)

    fragit_done = fragment6(fragit, 78 )
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Ordered_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    ...
    fragit_done.reverse()
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Reversed_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(fragit_done)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Mixed_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    
  def midstreamIPv6TcpQinQWrongTagInFragments(self, fragit, \
  results_directory, sid_id_tcp_v6, src_name, repo_name):
    #Wrongly tagged fragments 
    #Using DOUBLE VLAN Tagging - QinQ
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Ordered_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Reversed_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_dot1ad)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_wrong_dot1ad)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Mixed_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_dot1ad)
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Ordered_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Reversed_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_dot1q)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_wrong_dot1q)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Mixed_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_dot1q)
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Ordered_QinQ_data_frag_only_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Reversed_QinQ_data_frag_only_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_only_dot1q)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_only_dot1q)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Mixed_QinQ_data_frag_only_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_only_dot1q)
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Ordered_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Reversed_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_both)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_wrong_both)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Mixed_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_both)
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Ordered_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Reversed_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_missing_tags)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_missing_tags)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Mixed_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_missing_tags)
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Ordered_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Reversed_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_reversed_tags)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_reversed_tags)
    wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream_Fragmented_Mixed_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_tcp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_reversed_tags)
  ...
  def reconstructIPv6Packet(self, packet):
    ...
    sport=packet[TCP].sport, dport=55555, seq=packet.seq, \
    ack=packet.ack)/"RANDOM data 12345 - 007 Sexy and You know it 007"\
    /packet[TCP][Raw]
  ...
  source_name, sid_id_tcp_v6, tcp_str, repository_name):
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['WriteRule']:
      self.writeIPv6TcpRule(sid_id_tcp_v6, tcp_str, \
      os.path.join(results_directory, 'Rules'), source_name)
    
    if Global_Vars.yaml_options['Protocols']['TCP']['Midstream']['Midstream']:
      wrpcap("%s/%s-%s-%s_IPv6_TCP_Midstream-%s-tp-01.pcap" \
      % (os.path.join(results_directory, 'Midstream', 'Regular'), sid_id_tcp_v6, self.incrementPcapId("byOne"), \
      source_name, repository_name) , ipv6_ready)
      
      self.midstreamIPv6Tcp(ipv6_frag_ready, results_directory, sid_id_tcp_v6, \
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Midstream']['Dot1Q']:
      self.midstreamIPv6TcpDot1Q(ipv6_frag_ready, results_directory, \
      sid_id_tcp_v6, source_name, repository_name)
      
      self.midstreamIPv6TcpDot1QWrongTagInFragments(ipv6_frag_ready, \
      results_directory, sid_id_tcp_v6, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Midstream']['QinQ']:
      self.midstreamIPv6TcpQinQ(ipv6_frag_ready, results_directory, \
      sid_id_tcp_v6, source_name, repository_name)
      
      self.midstreamIPv6TcpQinQWrongTagInFragments(ipv6_frag_ready, \
      results_directory, sid_id_tcp_v6, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Session']['Session']:
      self.rebuildIPv6TcpSession(ipv6_ready, results_directory, sid_id_tcp_v6, \
      source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Session']['ExtraTcpSA']:
      self.rebuildIPv6TcpSessionExtraTcpSAs(ipv6_ready, results_directory, \
      sid_id_tcp_v6, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Session']['Dot1Q']:
      self.rebuildIPv6TcpSessionDot1Q(ipv6_ready, results_directory, \
      sid_id_tcp_v6, source_name, repository_name)
      
      self.rebuildIPv6TcpSessionDot1QWrongTagInFragments(ipv6_ready, \
      results_directory, sid_id_tcp_v6, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Session']['QinQ']:
      self.rebuildIPv6TcpSessionQinQ(ipv6_ready, results_directory, \
      sid_id_tcp_v6, source_name, repository_name)
      
      self.rebuildIPv6TcpSessionQinQWrongTagInFragments(ipv6_ready, \
      results_directory, sid_id_tcp_v6, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Session']['SeqOverspill']:
      self.rebuildIPv6TcpSeqOverSpill(ipv6_ready, results_directory, \
      sid_id_tcp_v6, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Session']['Dot1Q']:
      self.rebuildIPv6TcpSeqOverSpillDot1Q(ipv6_ready, results_directory, \
      sid_id_tcp_v6, source_name, repository_name)
      
      self.rebuildIPv6TcpSeqOverSpillDot1QWrongTagInFragments(ipv6_ready, \
      results_directory, sid_id_tcp_v6, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['TCP']['Session']['QinQ']:
      self.rebuildIPv6TcpSeqOverSpillQinQ(ipv6_ready, results_directory, \
      sid_id_tcp_v6, source_name, repository_name)
      
      self.rebuildIPv6TcpSeqOverSpillQinQWrongTagInFragments(ipv6_ready, \
      results_directory, sid_id_tcp_v6, source_name, repository_name)
