...
class pacifyIpv6Icmp:
  
  
  def writeIPv6IcmpRule(self, sid_id_icmp_v6, icmp_content_match_string, \
  directory, src_name):
    ##creating and writing a sid.rules file
    rule_file = open('%s/%s.rules' % (directory,sid_id_icmp_v6), 'w+')
    ...
    (sid_id_icmp_v6, sid_id_icmp_v6, content_all_ready_for_rule, src_name, \
    sid_id_icmp_v6) )
    
    rule_file.close()


  def rebuildIPv6IcmpSession(self, packet, results_directory, \
  sid_id_icmp_v6, src_name, repo_name):
    ...
    /IPv6(src=ipsrc, dst=ipdst)/ICMPv6EchoRequest(id=packet[ICMPv6EchoReply].id, \
    seq=(packet[ICMPv6EchoReply].seq - 1 ), data=self.icmp_Random_Data() )
    
    ICMP2 =  Ether(src=packet[Ether].dst, dst=packet[Ether].src ) \
    /IPv6(src=ipdst, dst=ipsrc)/ICMPv6EchoReply(id=packet[ICMPv6EchoReply].id, \
    seq=(packet[ICMPv6EchoReply].seq - 1), data=ICMP1[ICMPv6EchoRequest].data )
    
    ICMP3 =  Ether(src=packet[Ether].src, dst=packet[Ether].dst ) \
    /IPv6(src=ipsrc, dst=ipdst)/ICMPv6EchoRequest(id=packet[ICMPv6EchoReply].id, \
    seq=packet[ICMPv6EchoReply].seq, data=packet[ICMPv6EchoReply].data )
    ...
    p_to_be_fraged = Ether(src=packet[Ether].dst, dst=packet[Ether].src ) \
    /IPv6(src=ipdst, dst=ipsrc)/IPv6ExtHdrFragment()/ICMPv6EchoReply( \
    id=packet[ICMPv6EchoReply].id, seq=packet[ICMPv6EchoReply].seq, \
    data=packet[ICMPv6EchoReply].data )
    
    p_frag = fragment6( p_to_be_fraged , 74)
    
    ICMP5 = Ether(src=packet[Ether].src, dst=packet[Ether].dst ) \
    /IPv6(src=ipsrc, dst=ipdst)/ICMPv6EchoRequest(id=packet[ICMPv6EchoReply].id, \
    seq=(packet[ICMPv6EchoReply].seq + 1 ), data=self.icmp_Random_Data() )
    
    ICMP6 =  Ether(src=packet[Ether].dst, dst=packet[Ether].src ) \
    /IPv6(src=ipdst, dst=ipsrc)/ICMPv6EchoReply(id=packet[ICMPv6EchoReply].id, \
    seq=(packet[ICMPv6EchoReply].seq + 1), data=ICMP5[ICMPv6EchoRequest].data )
    
    ICMP7 =  Ether(src=packet[Ether].src, dst=packet[Ether].dst ) \
    /IPv6(src=ipsrc, dst=ipdst)/ICMPv6EchoRequest(id=packet[ICMPv6EchoReply].id, \
    seq=(packet[ICMPv6EchoReply].seq + 2 ), data=self.icmp_Random_Data() )
    ...
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    ...
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'Regular'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list


  def rebuildIPv6IcmpSessionDot1Q(self, packet, results_directory, \
  sid_id_icmp_v6, src_name, repo_name):
    ...
    ##This is the actual data packet that will be send, containing the payload
    p = Ether(src=packet[Ether].dst, dst=packet[Ether].src ) \
    /IPv6(src=ipsrc, dst=ipdst)/ICMPv6EchoReply(id=packet[ICMPv6EchoReply].id, \
    seq=(packet[ICMPv6EchoReply].seq ), data=packet[ICMPv6EchoReply].data )
    ...
    session_packets.append(ICMP6)
    session_packets.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Dot1Q-%s-tp-02.pcap" 
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered_Dot1Q-%s-tp-02.pcap"\
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed_Dot1Q-%s-tp-02.pcap"\
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed_Dot1Q-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(ICMP6)
    session_packets.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Dot1Q_tagged_wrong-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered_Dot1Q_tagged_wrong-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed_Dot1Q_tagged_wrong-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed_Dot1Q_tagged_wrong-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(ICMP6)
    session_packets.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Dot1Q_data_tag_missing-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered_Dot1Q_data_tag_missing-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed_Dot1Q_data_tag_missing-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed_Dot1Q_data_tag_missing-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list


  def rebuildIPv6IcmpSessionDot1QWrongTagInFragments(self, packet, \
  results_directory, sid_id_icmp_v6, src_name, repo_name):
    ...
    /IPv6(src=ipsrc, dst=ipdst)/IPv6ExtHdrFragment()/ICMPv6EchoReply( \
    id=packet[ICMPv6EchoReply].id, seq=packet[ICMPv6EchoReply].seq, \
    data=packet[ICMPv6EchoReply].data )
    p_Dot1Q_data_frag.tags = Dot1Q(vlan=1111)
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered_Dot1Q_data_tag_missing_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed_Dot1Q_data_tag_missing_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed_Dot1Q_data_tag_missing_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered_Dot1Q_data_tag_wrong_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed_Dot1Q_data_tag_wrong_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed_Dot1Q_data_tag_wrong_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered_Dot1Q_data_tag_one_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed_Dot1Q_data_tag_one_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed_Dot1Q_data_tag_one_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
  ...
  def rebuildIPv6IcmpSessionQinQ(self, packet, results_directory, \
  sid_id_icmp_v6, src_name, repo_name):
    ...
    session_packets.append(ICMP6)
    session_packets.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_QinQ-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered_QinQ-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed_QinQ-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed_QinQ-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(ICMP6)
    session_packets.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_QinQ_tags_reversed-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered_QinQ_tags_reversed-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed_QinQ_tags_reversed-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed_QinQ_tags_reversed-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets.append(ICMP6)
    session_packets.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_QinQ_data_tag_missing-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets)
    session_packets[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered_QinQ_data_tag_missing-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed_QinQ_data_tag_missing-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed_QinQ_data_tag_missing-%s-tp-02.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list

  def rebuildIPv6IcmpSessionQinQWrongTagInFragments(self, packet, \
  results_directory, sid_id_icmp_v6, src_name, repo_name):
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered_QinQ_data_frag_wrong_dot1q_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed_QinQ_data_frag_wrong_dot1q_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed_QinQ_data_frag_wrong_dot1q_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed_QinQ_data_frag_wrong_dot1ad_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered_QinQ_data_frag_wrong_tags_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed_QinQ_data_frag_wrong_tags_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed_QinQ_data_frag_wrong_tags_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered_QinQ_data_frag_missing_tags_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed_QinQ_data_frag_missing_tags_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed_QinQ_data_frag_missing_tags_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered_QinQ_data_frag_reversed_tags_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed_QinQ_data_frag_reversed_tags_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed_QinQ_data_frag_reversed_tags_in_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Ordered_QinQ_data_frag_one_tagged_fragments-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Reversed_QinQ_data_frag_one_tagged_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
    ...
    session_packets_fragmented.append(ICMP6)
    session_packets_fragmented.append(ICMP7)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Session_Fragmented_Mixed_QinQ_data_frag_one_tagged_fragment-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), session_packets_fragmented)
    session_packets_fragmented[:] = [] #empty the list
  ...
  def midstreamIPv6Icmp(self, fragit, results_directory, sid_id_icmp_v6, \
  src_name, repo_name):
    
    #forcing correct recalculation of the checksum
    fragit[IPv6].chksum=None
    
    
    fragit_done = fragment6(fragit, 74 )
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Ordered-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Regular'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    ...
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Reversed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Regular'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(fragit_done)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Mixed-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Regular'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    
  def midstreamIPv6IcmpDot1Q(self, fragit, results_directory, \
  sid_id_icmp_v6, src_name, repo_name):
    ...
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit)
    
    fragit_done = fragment6(fragit, 74 )
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Ordered_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    ...
    fragit_done.reverse()
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Reversed_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(fragit_done)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Mixed_Dot1Q-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    
  def midstreamIPv6IcmpDot1QWrongTagInFragments(self, fragit, \
  results_directory, sid_id_icmp_v6, src_name, repo_name):
    ...
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Ordered_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Reversed_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done_wrong_dot1q_tag)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(fragit_done_wrong_dot1q_tag)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Mixed_Dot1Q_data_tag_wrong_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done_wrong_dot1q_tag)
    ...
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Ordered_Dot1Q_data_tag_none_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Reversed_Dot1Q_data_tag_none_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done_no_dot1q_tag)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(fragit_done_no_dot1q_tag)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Mixed_Dot1Q_data_tag_none_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'Dot1Q'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done_no_dot1q_tag)


  def midstreamIPv6IcmpQinQ(self, fragit, results_directory, \
  sid_id_icmp_v6, src_name, repo_name):
    ...
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit)

    fragit_done = fragment6(fragit, 78 )
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Ordered_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    ...
    fragit_done.reverse()
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Reversed_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(fragit_done)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Mixed_QinQ-%s-tp-01.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), fragit_done)
    
    
  def midstreamIPv6IcmpQinQWrongTagInFragments(self, fragit, \
  results_directory, sid_id_icmp_v6, src_name, repo_name):
    #Wrongly tagged fragments 
    #Using DOUBLE VLAN Tagging - QinQ
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Ordered_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Reversed_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_dot1ad)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_wrong_dot1ad)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Mixed_QinQ_data_frag_wrong_dot1ad_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_dot1ad)
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Ordered_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Reversed_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_dot1q)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_wrong_dot1q)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Mixed_QinQ_data_frag_wrong_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_dot1q)
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Ordered_QinQ_data_frag_only_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Reversed_QinQ_data_frag_only_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_only_dot1q)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_only_dot1q)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Mixed_QinQ_data_frag_only_dot1q_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_only_dot1q)
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Ordered_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Reversed_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_both)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_wrong_both)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Mixed_QinQ_data_frag_wrong_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_wrong_both)
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Ordered_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Reversed_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_missing_tags)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_missing_tags)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Mixed_QinQ_data_frag_missing_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_missing_tags)
    ...
    ##
    #write the ordered fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Ordered_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    ...
    #write the reversed fragmented payload packet and write
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Reversed_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_reversed_tags)
    
    #shuffle(unorder/mix) the fragmented payload packet and write
    random.shuffle(p_frag_QinQ_data_frag_reversed_tags)
    wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream_Fragmented_Mixed_QinQ_data_frag_reversed_tags_in_fragment-%s-fp-00.pcap" \
    % (os.path.join(results_directory, 'Midstream', 'QinQ'), sid_id_icmp_v6, self.incrementPcapId("byOne") \
    , src_name, repo_name), p_frag_QinQ_data_frag_reversed_tags)

  def reconstructIPv6IcmpPacket(self, packet):
    # here we make the original HTTP packet into a just IPv6 packet
    ...
    p = Ether(src=packet[Ether].dst, dst=packet[Ether].src ) \
    /IPv6(src=ipsrc, dst=ipdst)/ICMPv6EchoReply(id=12345, seq=54321, \
    data=packet[TCP][Raw] )
    
    return p

  def reconstructIPv6IcmpPacketFragmentationReady(self, packet):
    ...
    p = Ether(src=packet[Ether].dst, dst=packet[Ether].src ) \
    /IPv6(src=ipsrc, dst=ipdst)/IPv6ExtHdrFragment()/ICMPv6EchoReply(id=12345, \
    seq=54321, data=packet[TCP][Raw] )
  ...
  source_name, sid_id_icmp_v6, icmp_str, repository_name):
    ...
    ipv6_icmp_packet = self.reconstructIPv6IcmpPacket(scapy_load[FN])
    ipv6_icmp_packet_frag_ready = \
    self.reconstructIPv6IcmpPacketFragmentationReady(scapy_load[FN])
    
    if Global_Vars.yaml_options['Protocols']['ICMP']['WriteRule']:
      self.writeIPv6IcmpRule(sid_id_icmp_v6, icmp_str, \
    ...
    if Global_Vars.yaml_options['Protocols']['ICMP']['PseudoMidstream']['PseudoMidstream']:
      wrpcap("%s/%s-%s-%s_IPv6_ICMP_Midstream-%s-tp-01.pcap" \
      % (os.path.join(results_directory, 'Midstream', 'Regular'), sid_id_icmp_v6, self.incrementPcapId("byOne"), \
      source_name, repository_name) , ipv6_icmp_packet)
      
      self.midstreamIPv6Icmp(ipv6_icmp_packet_frag_ready, results_directory, \
      sid_id_icmp_v6, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['ICMP']['PseudoMidstream']['Dot1Q']:
      self.midstreamIPv6IcmpDot1Q(ipv6_icmp_packet_frag_ready, \
      results_directory, sid_id_icmp_v6, source_name, repository_name)
      
      self.midstreamIPv6IcmpDot1QWrongTagInFragments(ipv6_icmp_packet_frag_ready, \
      results_directory, sid_id_icmp_v6, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['ICMP']['PseudoMidstream']['QinQ']:
      self.midstreamIPv6IcmpQinQ(ipv6_icmp_packet_frag_ready, \
      results_directory, sid_id_icmp_v6, source_name, repository_name)
      
      self.midstreamIPv6IcmpQinQWrongTagInFragments(ipv6_icmp_packet_frag_ready, \
      results_directory, sid_id_icmp_v6, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['ICMP']['PseudoSession']['PseudoSession']:
      self.rebuildIPv6IcmpSession(ipv6_icmp_packet, results_directory, \
      sid_id_icmp_v6, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['ICMP']['PseudoSession']['Dot1Q']:
      self.rebuildIPv6IcmpSessionDot1Q(ipv6_icmp_packet, results_directory, \
      sid_id_icmp_v6, source_name, repository_name)
      
      self.rebuildIPv6IcmpSessionDot1QWrongTagInFragments(ipv6_icmp_packet, \
      results_directory, sid_id_icmp_v6, source_name, repository_name)
    ...
    if Global_Vars.yaml_options['Protocols']['ICMP']['PseudoSession']['QinQ']:
      self.rebuildIPv6IcmpSessionQinQ(ipv6_icmp_packet, results_directory, \
      sid_id_icmp_v6, source_name, repository_name)
      
      self.rebuildIPv6IcmpSessionQinQWrongTagInFragments(ipv6_icmp_packet, \
      results_directory, sid_id_icmp_v6, source_name, repository_name)
